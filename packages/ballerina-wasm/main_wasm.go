...
func run(_ js.Value, args []js.Value) any {
	return newPromise(func(resolve js.Value, _ js.Value) {
		go func() {
			...
			if err != nil {
				fmt.Fprintf(stderr, "%v\n", err)
				resolve.Invoke(js.ValueOf(map[string]any{"error": err.Error()}))
				return
			}
...
func stop(_ js.Value, args []js.Value) any {
	signal := pal.GracefulStop
	if len(args) > 0 && args[0].Type() == js.TypeString && args[0].String() == "immediate" {
		signal = pal.ImmediateStop
	}
	return activeRun.sendSignal(signal)
}
